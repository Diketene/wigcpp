...
static_assert(sizeof_prime_list_item == 2 || sizeof_prime_list_item == 4,
              "Invalid sizeof_prime_list_item, must be 2 or 4");
...
template <unsigned size> struct multi_word_traits {};

#if defined(__SIZEOF_INT128__) && (__SIZEOF_INT128__ == 16)

constexpr inline unsigned sizeof_mwi_item = 8;

template <> struct multi_word_traits<8> {
...
template <> struct multi_word_traits<4> {
...
template <typename T> constexpr inline uword_t full_sign_word(T x) {
...
namespace prime {
template <unsigned N> struct prime_exp_traits {};

template <> struct prime_exp_traits<4> {
  using prime_exp_t = std::int32_t;
  using uprime_exp_t = std::uint32_t;
};

template <> struct prime_exp_traits<2> {
