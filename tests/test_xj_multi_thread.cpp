...
TEST(test_xj_thread, SingleFunc_3j) {
  {
    ...
    for (int i = 0; i < kThreads; ++i) {
      threads.emplace_back(
          [=, &result] { result[i] = wigcpp::three_j(two_j1, two_j2, two_j3, two_m1, two_m2, two_m3); });
    }
...

  {
    ...
    for (int i = 0; i < kThreads; ++i) {
      threads.emplace_back([&thread_data, i] {
        thread_data[i].result = wigcpp::three_j(thread_data[i].two_j1, thread_data[i].two_j2, thread_data[i].two_j3,
