...
namespace wigcpp::internal::global {
  ...
  struct PrimeTable{
    ...
    PrimeTable(int max_factorial) noexcept : prime_list{}, num_primes{0}, max_factorial(max_factorial){
  ...
  public:
    FactorPool(const PrimeTable &prime_table): aligned_block_bytes{}, buffer{}{
      const std::size_t bytes_of_prime_exponents = sizeof(int) + sizeof(exp_t) * prime_table.num_primes;
      const std::size_t aligned_bytes = (bytes_of_prime_exponents + 63) & ~static_cast<std::size_t>(63);
      aligned_block_bytes = aligned_bytes;
      buffer.resize((prime_table.max_factorial + 1) * aligned_bytes, std::byte{0});
  ...
  public:
    GlobalFactorialPool(int max_factorial) noexcept: prime_table(max_factorial),
        num_pool(prime_table), factorial_pool(prime_table)
        {
          fill_num_pool(prime_table);
          fill_factorial_pool(max_factorial, prime_table.num_primes);
        }
